...
export function VerificationsPageSkeleton() {
  return (
    <div
      ...
      role="status"
      aria-busy="true"
      aria-label="Loading verifications overview"
    >
      <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <SkeletonBone className="h-8 w-32 rounded-md" />
        </div>
      </div>
      ...
      <div className="flex gap-2 overflow-hidden border-b border-border-subtle pb-0">
        {Array.from({ length: 6 }).map((_, index) => (
          <SkeletonBone key={index} className="h-8 w-24 rounded-none" />
        ))}
      </div>
