...
export type OperationsVerificationTab =
  | "all"
  | "pending"
  | "approved"
  | "rejected";

export interface OperationsVerificationTabCounts {
  all: number;
  pending: number;
  approved: number;
  rejected: number;
...
interface VerificationsTabsProps {
  activeTab: OperationsVerificationTab;
  counts: OperationsVerificationTabCounts;
  onChange: (tab: OperationsVerificationTab) => void;
}

const TABS: {
  id: OperationsVerificationTab;
  label: string;
  shortLabel: string;
}[] = [
...
export function VerificationsTabs({
  ...
  return (
    <div
      ...
      role="tablist"
      aria-label="Employer verification tabs"
    >
      {TABS.map((tab) => {
