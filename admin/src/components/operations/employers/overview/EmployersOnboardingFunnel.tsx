...
export function EmployersOnboardingFunnel({
  ...
  return (
    <section className="operations-density-card flex h-full min-w-0 flex-col rounded-xl border border-border-subtle bg-surface shadow-sm">
      <header className="flex shrink-0 items-center justify-between gap-2 px-3 pt-2.5 sm:px-3.5 sm:pt-3">
      ...
      <div className="min-h-0 flex-1 px-3 pb-3 pt-3 sm:px-3.5 sm:pb-3.5">
        {isLoading ? (
          <div className="flex min-h-[11rem] flex-col justify-center gap-3" aria-hidden="true">
            {Array.from({ length: 5 }).map((_, index) => (
              <div
                key={index}
                className="grid animate-pulse grid-cols-[7.5rem_minmax(0,1fr)_3.25rem_2.25rem] items-center gap-2 sm:grid-cols-[8.75rem_minmax(0,1fr)_3.5rem_2.5rem]"
              >
                <div className="h-3 rounded bg-hero-bg" />
                <div className="h-2.5 rounded-full bg-hero-bg" />
                <div className="h-3 rounded bg-hero-bg" />
                <div className="h-3 rounded bg-hero-bg" />
              </div>
            ))}
          ...
          <ul className="flex min-h-[11rem] flex-col justify-center gap-2.5">
            {displayStages.map((stage, index) => {
              ...
              return (
                <li
                  key={stage.id}
                  className="grid grid-cols-[7.5rem_minmax(0,1fr)_3.25rem_2.25rem] items-center gap-x-2 sm:grid-cols-[8.75rem_minmax(0,1fr)_3.5rem_2.5rem] sm:gap-x-2.5"
                >
                  ...
                  <div
                    className="h-2.5 w-full overflow-hidden rounded-full bg-[#EEF2F6]"
                    role="progressbar"
                  ...
                  <span
                    className={cn(
                      "text-right text-[11px] font-semibold tabular-nums sm:text-[12px]",
                      tone === "verified"
                        ? "text-success"
                        : "text-foreground",
                    )}
                  ...
                  <span
                    className={cn(
                      "text-right text-[11px] tabular-nums sm:text-[12px]",
                      tone === "verified" ? "text-success" : "text-muted",
                    )}
                  >
                    {percent}%
                  </span>
