    ...
    >
      {chartData.length === 0 || total === 0 ? (
        ...
        <div className="flex min-h-44 min-w-0 flex-col gap-4 max-sm:min-h-36 max-sm:gap-3 sm:flex-row sm:items-center xl:min-h-36 xl:gap-3">
          <div className="relative mx-auto size-36 shrink-0 max-sm:size-32 xl:size-28">
            ...
            <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center text-center">
              <span className="text-xl font-bold leading-none text-foreground max-sm:text-lg xl:text-lg">
          ...
          <ul className="min-w-0 flex-1 space-y-1.5 xl:space-y-1">
            {chartData.map((item, index) => (
              <li key={item.id} className="flex items-center justify-between gap-2 text-[11px] max-sm:text-[10px] xl:text-[10px]">
