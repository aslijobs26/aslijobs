  ...
  return (
        ...
        <div className="flex flex-wrap gap-1" role="tablist" aria-label="Priority filters">
          {TABS.map((tab) => (
            <button
              ...
              className={cn(
                "rounded px-1.5 py-0.5 text-[10px] font-medium transition-colors sm:text-[11px]",
                activeTab === tab.id
    ...
    >
      <div className="overflow-x-auto overscroll-x-contain scrollbar-hidden">
        <table className="w-full min-w-[600px] border-collapse text-left text-[11px] leading-tight">
          <thead>
            <tr className="border-b border-border-subtle bg-hero-bg/50 text-muted">
              ...
              <th className="px-2 py-1.5 font-medium">Lang</th>
            ...
            {filteredItems.map((item) => (
              ...
              >
                ...
                <td className="px-2 py-1.5">
                  <OperationsBadge
                    variant={entityBadgeVariant(item.type)}
                    className="px-1.5 py-0 text-[10px]"
                  >
                    {item.type.charAt(0).toUpperCase() + item.type.slice(1)}
                  </OperationsBadge>
                </td>
                <td className="max-w-[6.5rem] truncate px-2 py-1.5 text-foreground">
                  {item.customer}
                </td>
                <td className="max-w-[8rem] truncate px-2 py-1.5 text-muted">{item.task}</td>
                <td className="px-2 py-1.5">
                  <OperationsBadge
                ...
                <td className="px-2 py-1.5 text-muted">{item.language}</td>
                <td className="px-2 py-1.5">
                  {item.assignedTo ? (
                    <span className="inline-flex items-center gap-1">
                      <OperationsAvatar
                        initials={item.assignedTo.initials}
                        className="size-5 text-[9px]"
                      />
                      <span className="max-w-[3.5rem] truncate text-muted">
