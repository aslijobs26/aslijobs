...
import type { OperationsPlacementsNamedCount } from "../../../../types/operations-placements";
...
interface PlacementsByLocationProps {
  items: OperationsPlacementsNamedCount[];
  isLoading?: boolean;
  isError?: boolean;
  onRetry?: () => void;
}
...
export function PlacementsByLocation({
  items,
  isLoading = false,
  isError = false,
  onRetry,
}: PlacementsByLocationProps) {
          ...
          <div className="grid min-h-[11.5rem] grid-cols-1 items-center gap-3 sm:grid-cols-2 xl:min-h-36 xl:gap-2">
            <div className="relative mx-auto w-full max-w-[11.5rem] max-sm:max-w-[9.5rem] sm:max-w-none">
              <svg
                ...
                role="img"
                aria-label="India map of placement concentration by state"
              >
                {mapFeatures.map((feature) => {
                  ...
                  return (
                    <path
                      ...
                      tabIndex={0}
                      aria-label={`${canonical}: ${countByState.get(key) ?? 0} placements`}
                    />
                  );
                })}
              </svg>
              {tooltip ? (
                <div className="pointer-events-none absolute left-1/2 top-2 z-10 -translate-x-1/2 rounded-md border border-border-subtle bg-surface px-2 py-1 text-center shadow-sm">
                  ...
                  <p className="text-[10px] text-muted">
                    {tooltip.count.toLocaleString("en-IN")} Placements
                  </p>
                </div>
              ) : null}
            </div>
                  ...
                  <li
                    ...
                    className={cn(
                      ...
                      hoveredState?.toLowerCase() ===
                        item.label.toLowerCase() && "bg-hero-bg/70",
                    )}
