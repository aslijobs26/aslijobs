...
import {
  ...
  JOB_POSTED_SUCCESS_LIVE_CHANGE_HEADING_EMPHASIS,
  JOB_POSTED_SUCCESS_LIVE_CHANGE_HEADING_PREFIX,
  JOB_POSTED_SUCCESS_LIVE_CHANGE_SUBTITLE,
...
type JobPostedSuccessHeroProps = {
  isLiveChangeReview?: boolean;
};

export function JobPostedSuccessHero({
  isLiveChangeReview = false,
}: JobPostedSuccessHeroProps) {
  const headingPrefix = isLiveChangeReview
    ? JOB_POSTED_SUCCESS_LIVE_CHANGE_HEADING_PREFIX
    : JOB_POSTED_SUCCESS_HEADING_PREFIX;
  const headingEmphasis = isLiveChangeReview
    ? JOB_POSTED_SUCCESS_LIVE_CHANGE_HEADING_EMPHASIS
    : JOB_POSTED_SUCCESS_HEADING_EMPHASIS;
  const subtitle = isLiveChangeReview
    ? JOB_POSTED_SUCCESS_LIVE_CHANGE_SUBTITLE
    : JOB_POSTED_SUCCESS_SUBTITLE;
  ...
  return (
    ...
    >
      ...
      >
        {headingPrefix}{" "}
        <span className="text-primary-soft">{headingEmphasis}</span>
      </h1>
