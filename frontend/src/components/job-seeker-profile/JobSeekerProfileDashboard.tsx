...
export function JobSeekerProfileDashboard() {
    ...
    return (
      <div className="mx-auto w-full max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
        <div className="animate-pulse space-y-4">
          <div className="h-32 rounded-xl bg-primary-light/40" />
          <div className="h-10 w-full max-w-xl rounded bg-primary-light/30" />
          <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_18rem]">
            <div className="h-96 rounded-xl bg-primary-light/25" />
            <div className="h-80 rounded-xl bg-primary-light/25" />
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-4 py-6 sm:px-6 lg:py-8 lg:px-8">
      <header className="rounded-xl border border-border-subtle bg-surface p-4 sm:p-6">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
          <div className="flex min-w-0 flex-col gap-4 sm:flex-row sm:items-start">
            ...
            <div className="min-w-0">
              ...
              <ul className="mt-3 flex flex-col gap-1.5 text-sm text-muted sm:flex-row sm:flex-wrap sm:items-center sm:gap-x-4 sm:gap-y-1">
                {locationLabel ? (
              ...
              {tags.length > 0 ? (
                <ul className="mt-3 flex flex-wrap gap-2">
                  {tags.map((tag) => (
                    <li
                      key={tag}
                      className="rounded-full bg-primary-light px-2.5 py-1 text-xs font-semibold text-primary"
                    >
                      {tag}
                    </li>
                  ))}
          ...
          <div className="flex w-full shrink-0 items-center gap-3 rounded-xl border border-border-subtle bg-hero-bg/80 p-3 sm:w-auto sm:min-w-[13.5rem]">
            <ProfileStrengthCircle percentage={strength.percent} size={64} />
            <div className="min-w-0">
              <p className="text-sm font-bold text-foreground">
                {strength.percent}%
              </p>
              <p className="text-xs text-muted">{strength.message}</p>
              <button
                type="button"
                className="mt-1 text-xs font-semibold text-primary underline-offset-2 hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/30"
      ...
      <div className="mt-6 lg:grid lg:grid-cols-[minmax(0,1fr)_18.5rem] lg:items-start lg:gap-6">
        <div className="min-w-0">
          <div
          ...
          >
            <div className="flex min-w-max gap-1 border-b border-border-subtle pb-px">
              {JOB_SEEKER_PROFILE_TABS.map((tab) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "inline-flex min-h-10 shrink-0 items-center border-b-2 px-3 text-sm font-semibold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/30",
                      selected
                        ? "border-primary text-primary"
                        : "border-transparent text-muted hover:border-primary/30 hover:text-foreground",
                    )}
                    onClick={() => setTab(tab)}
                  >
